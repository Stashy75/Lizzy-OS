...
if check_password(): 
    ...
    # --- 5. CENTERED BOOT SEQUENCE (CALIBRATED) ---
    if not st.session_state.booted:
        ...
        with boot_area.container():
            ...
            # AUDIO TIMING FIX: Wait 2.5 seconds for chime to end before speaking
            time.sleep(2.5) 
            ...
            time.sleep(1.5) # Allow user to see 100% completion
    ...
    with tabs[1]: # VAULT
        ...
        with col2:
            ...
            intel_file = st.file_uploader("Upload Tactical Files")
            if intel_file:
    ...
    with tabs[3]: # COMM_LINK
        ...
        if u_in:
            ...
            completion = client.chat.completions.create(
                ...
                messages=[{"role": "system", "content": context}] + st.session_state.messages[-5:]
            )
            ans = completion.choices[0].message.content
            st.session_state.messages.append({"role": "assistant", "content": ans})
            lizzy_speak(ans) # This ensures every AI reply is spoken aloud
            st.rerun()